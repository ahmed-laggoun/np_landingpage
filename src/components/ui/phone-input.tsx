...
// 2. Clear out inner typing overrides to match standard forwardRef rules
  ...
  ({ className, onChange, ...props }, ref) => {
    return (
      <RPNInput.default
        ...
        // Fix: Route the forwardRef directly through the input renderer wrapper
        inputComponent={React.forwardRef<HTMLInputElement, React.ComponentProps<"input">>(
          ({ className, ...inputProps }, inputRef) => (
            <Input
              className={cn("rounded-e-md rounded-s-none", className)}
              ref={ref || inputRef} // Bind parent forwardRef directly to the DOM node
              {...inputProps}
            />
          )
        )}
